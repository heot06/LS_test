...
# EtherFos OPC UA endpoint 설정
ETHERFOS_IP = "192.168.0.100"   # 실제 EtherFos 장치 IP로 변경
ENDPOINT = f"opc.tcp://{ETHERFOS_IP}:4840"
...
class LSOPCUA:
    ...
    def connect(self):
        self.client.connect()
        print(f"[OPC UA] 서버 연결 성공: {ENDPOINT}")
...
try:
    # EtherFos 게이트웨이 하위에 있는 PLC 네임스페이스 경로로 수정 필요
...
async def plc_monitor():
    target_nodes = ["Axis1_100","Axis2_100","Axis3_100","Axis4_100","Axis5_100","Axis6_100",
                    "Dev_Proximity_sensor","Developing_conveyor_RPM","Developing_conveyor_flux",
                    "Developing_conveyor_temperature","Drying_conveyor_RPM","Drying_conveyor_flux",
                    "Drying_conveyor_temperature","Etching_conveyor_RPM","Etching_conveyor_flux",
                    "Etching_conveyor_temperature","Proximity_sensor","StrippingAlarm","Stripping_conveyor_RPM",
                    "Stripping_conveyor_flux","Stripping_conveyor_temperature"]  # 모니터링할 노드들
    while True:
